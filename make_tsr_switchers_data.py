...
with open('data/tsr_control/sentiments_control_pre.txt') as f:
	for line in f:
...
linenum = 1
with open('data/tsr_control/sentiments_control_post.txt') as f:
	for line in f:
		_, date_str, username, _, sentiment = line.split(" ::---:: ", 4)
		if username in ht:
			user = ht[username]
			...
			linenum += 1
#pdb.set_trace()
with open('data/tsr_control/switcher_control.txt', 'w') as f:
	for username in ht:
		to_write = username
		user = ht[username]
